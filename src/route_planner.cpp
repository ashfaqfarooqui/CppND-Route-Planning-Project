...
void RoutePlanner::AddNeighbors(RouteModel::Node *current_node) {
  ...
  for (auto nb : current_node->neighbors) {
    ...
    nb->g_value = current_node->g_value + nb->distance(*current_node);
...
RoutePlanner::ConstructFinalPath(RouteModel::Node *current_node) {
  ...
  auto cn = current_node;
  path_found.push_back(*cn);
  while (cn->parent != nullptr) {

    distance += cn->distance(*(cn->parent));
    cn = cn->parent;
    path_found.push_back(*cn);
  }
